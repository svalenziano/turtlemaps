    ...
    drawOSM(json) {
        ...
        for (let ele of elements) {
            ...
            if (ele.type === "way") {
                ...
                const geom = Overpass.extractRelationGeom(ele, pointTransformer);
                const path = SVG.makePath(geom);
                path.setAttribute("stroke", Color.default.hilite);
                layer.addGeometry(path);
            }
            else {
                const x = ele; // TS exhaustiveness check
            }
        }
        // 
    }
